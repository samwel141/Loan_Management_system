...
Class Action {
	...
	public function save_user() {
		...
		if(empty($id)) {
			$stmt = $conn->prepare("INSERT INTO users (name, username, password, type, doctor_id, company, contact) VALUES (?, ?, ?, ?, ?, ?, ?)");
			$stmt->bind_param("sssisss", $name, $username, $hashed_password, $type, $doctor_id, $company, $contact);
		} else {
			$stmt = $conn->prepare("UPDATE users SET name = ?, username = ?, password = ?, type = ?, doctor_id = ?, company = ?, contact = ? WHERE id = ?");
			$stmt->bind_param("sssisssi", $name, $username, $hashed_password, $type, $doctor_id, $company, $contact, $id);
		}
